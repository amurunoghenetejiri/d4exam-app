...
export async function switchToAccount(
  ...
  try {
    ...
    // Access expired or setSession failed → refresh once and SAVE immediately
    ...
    // Nuclear fallback: inject storage and hard navigate (supabase picks up on load)
    injectSessionIntoStorage(accessTok, refreshTok, userPayload);
    setActiveAccountId(targetId);
    if (account.role) seedPendingLoginRole(account.role);
    if (typeof window !== "undefined") {
      window.location.replace(path);
