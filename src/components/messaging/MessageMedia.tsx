    ...
    <div className="fixed inset-0 z-[90] flex flex-col bg-black"
      ...
      onTouchMove={(e) => {
        if (e.touches.length === 2 && pinchRef.current) {
          e.preventDefault();
          const d = Math.hypot(e.touches[0].clientX - e.touches[1].clientX, e.touches[0].clientY - e.touches[1].clientY);
          const next = Math.max(1, Math.min(4, pinchRef.current.scale * (d / Math.max(1, pinchRef.current.dist))));
          setScale(next);
        }
      }}
  ...
  return (
    <div className="fixed inset-0 z-[90] flex flex-col bg-black">
      <div className="absolute inset-x-0 top-0 z-10 flex items-center justify-between px-3 py-3 pt-[max(0.75rem,env(safe-area-inset-top))]">
        ...
        <p className="rounded-full bg-black/50 px-3 py-1 text-sm font-semibold text-white backdrop-blur-sm">Video</p>
